...
describe('Contract: NFT', () => {
	beforeEach(async () => {
		...
		await nft.createToken(user0.address,user0.address, "metadata-url.com/my-metadata_1", 500)
		await nft.createToken(user0.address,user0.address, "metadata-url.com/my-metadata_2", 500)
		await nft.createToken(user1.address,user1.address, "metadata-url.com/my-metadata_3", 500)
	})
